...
2. **knowledge_retrieve**: 从知识库中检索相关信息，参数: {{"query": "检索问题"}}
   ...
   **重要规则**:
   ...
   - 如果代码较长，分多次调用 code_interpreter 执行，但每次都必须包含完整的数据加载步骤。
...
""".strip()
    ...
    while True:
        ...
        if event_type == "thinking":
            ...
            # Buffer thinking chunks for delayed step creation
            round_num = int(event.get("round") or (len(round_step_map) + 1))
            delta_thinking = event.get("delta_thinking") or ""
            delta_text = event.get("delta_text") or ""

            # Buffer the chunk, don't emit yet
            if round_num not in pending_thoughts:
                pending_thoughts[round_num] = []
            if delta_thinking:
                pending_thoughts[round_num].append(delta_thinking)
            elif delta_text:
                pending_thoughts[round_num].append(delta_text)
            # Step will be created when 'act' event arrives
            ...
            # Emit buffered thoughts as step.thought attached to the PREVIOUS step
            # (or "initial" if this is the first round)
