...
const Playground: NextPage = () => {
  ...
  const handleStart = async (inputQuery = query) => {
    if ((!inputQuery.trim() && !uploadedFile) || loading) return;
        ...
        if (payload.type === 'step.start') {
          ...
          setExecutionMap(prev => {
            ...
            const nextSteps = current.steps.map(item => {
              ...
              if (payload.thought) parts.push(`Thought: ${payload.thought}`);
              if (payload.action) {
  ...
  const QuickAction = ({ icon, text, onClick }: { icon: any; text: string; onClick?: () => void }) => (
    <div
      onClick={onClick}
            ...
            <div className='flex-1 flex overflow-hidden'>
              ...
              >
                <div className='flex-1 overflow-y-auto'>
                  {(() => {
                    const lastHumanMsg = [...messages].reverse().find(m => m.role === 'human');
                    const lastViewMsg = [...messages].reverse().find(m => m.role === 'view');
                    const execution = lastViewMsg?.id ? executionMap[lastViewMsg.id] : undefined;
                    const {
                      sections,
                      activeStep: _activeStep,
                      outputs: _outputs,
                      stepThoughts,
                    } = convertToManusFormat(execution, lastHumanMsg?.context);
                    const isWorking =
                      lastViewMsg?.thinking || execution?.steps.some(s => s.status === 'running') || false;

                    const handleStepClick = (stepId: string, _sectionId: string) => {
                      setSelectedStepId(stepId);
                      if (lastViewMsg?.id && execution) {
                        setExecutionMap(prev => ({
                          ...prev,
                          [lastViewMsg.id!]: {
                            ...prev[lastViewMsg.id!],
                    ...
                    return (
                      <ManusLeftPanel
                        sections={sections}
                        activeStepId={selectedStepId || execution?.activeStepId}
                        onStepClick={handleStepClick}
                        isWorking={isWorking}
                        userQuery={lastHumanMsg?.context}
                        attachedFile={lastHumanMsg?.attachedFile}
                        attachedKnowledge={lastHumanMsg?.attachedKnowledge}
                        assistantText={streamingSummary || lastViewMsg?.context || undefined}
                        modelName={lastViewMsg?.model_name || model}
                        stepThoughts={stepThoughts}
                        artifacts={artifacts.filter(a => a.messageId === lastViewMsg?.id)}
            ...
            <div className='flex-1 flex flex-col items-center justify-center p-4 pb-32 overflow-y-auto'>
              <div className='w-full max-w-3xl flex flex-col items-center animate-fade-in-up'>
                ...
                <h1 className='text-4xl md:text-5xl font-serif text-gray-900 dark:text-gray-100 mb-12 text-center flex items-center gap-4'>
                  ...
                  {t('analyze_your_data') || 'How can I analyze your data today?'}
                </h1>
                ...
                <div className='mt-8 flex flex-wrap justify-center gap-3'>
                  <QuickAction
                    icon={<TableOutlined />}
                    text='Analyze Excel'
                    onClick={() => setQuery('Analyze this Excel file and find trends...')}
                  />
                  <QuickAction
                    icon={<DatabaseOutlined />}
                    text='Query Database'
                    onClick={() => setQuery('Write a SQL query to find top users...')}
                  />
                  <QuickAction
                    icon={<BarChartOutlined />}
                    text='Generate Report'
                    onClick={() => setQuery('Create a visualization for monthly sales...')}
                  />
                  <QuickAction
                    icon={<PieChartOutlined />}
                    text='Data Visualization'
                    onClick={() => setQuery('Plot a pie chart of user distribution...')}
                  />
                </div>
              </div>
            </div>
